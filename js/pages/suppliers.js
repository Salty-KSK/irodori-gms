// ============================================================
// GMS - Suppliers Page (莉募・繧悟・繝ｪ繧ｹ繝・
// ============================================================
...
export class SuppliersPage {
  constructor() {
    ...
    this.categories = ['蜈ｨ縺ｦ', '繧ｫ繝｡繝ｩ讖滓攝', '辣ｧ譏・, '閭梧勹繝ｻ蟆冗黄', '蜊ｰ蛻ｷ', 'PC繝ｻ繧ｽ繝輔ヨ', '縺昴・莉・];
  }
      ...
      <div class="page-suppliers animate-fade-in-up">
        <div class="page-header">
          <div class="page-header-left">
            <h1 class="page-title">莉募・繧悟・繝ｪ繧ｹ繝・/h1>
            <p class="page-subtitle">蜿門ｼ募・繝ｻ莉募・繧悟・縺ｮ邂｡逅・/p>
          </div>
        ...
        <div class="sup-filter-section">
          ...
          <div class="search-bar sup-search hidden-mobile">
            ...
            <input type="text" placeholder="莉募・繧悟・繧呈､懃ｴ｢..." id="supSearchInput">
          </div>
        </div>
        ...
        <div class="data-table-wrapper hidden-mobile">
          <div class="data-table-toolbar">
            <div class="data-table-toolbar-left">
              <span class="text-secondary" id="supCount">0莉ｶ</span>
            </div>
          </div>
          <div style="overflow-x: auto;">
            <table class="data-table" id="supTable">
              <thead>
                <tr>
                  <th data-sort="name">莨夂､ｾ蜷・<span class="material-symbols-outlined sort-icon">arrow_upward</span></th>
                  <th data-sort="category">繧ｫ繝・ざ繝ｪ <span class="material-symbols-outlined sort-icon">arrow_upward</span></th>
                  <th data-sort="contactPerson">諡・ｽ楢・<span class="material-symbols-outlined sort-icon">arrow_upward</span></th>
                  <th>髮ｻ隧ｱ</th>
                  <th>Web</th>
                  <th data-sort="rating">隧穂ｾ｡ <span class="material-symbols-outlined sort-icon">arrow_upward</span></th>
                  <th>謫堺ｽ・/th>
                </tr>
              </thead>
        ...
        <div class="mobile-search  mb-4">
          <div class="flex gap-2">
            <div class="search-bar" style="flex:1;">
              <span class="material-symbols-outlined">search</span>
              <input type="text" placeholder="莉募・繧悟・繧呈､懃ｴ｢..." id="supSearchInputMobile">
            </div>
  ...
  bindEvents() {
    ...
    delegate('#supTableBody', '.sup-delete-btn', 'click', async (e, target) => {
      ...
      const item = store.getById('suppliers', id);
      if (!item) return;
      const confirmed = await confirmDialog('莉募・繧悟・縺ｮ蜑企勁', `縲・{item.name}縲阪ｒ蜑企勁縺励∪縺吶°・溘％縺ｮ謫堺ｽ懊・蜿悶ｊ豸医○縺ｾ縺帙ｓ縲Ａ);
      if (confirmed) {
        store.delete('suppliers', id);
        showSnackbar('莉募・繧悟・繧貞炎髯､縺励∪縺励◆');
        this.loadData();
      }
    });
        ...
        <tr data-id="${s.id}">
          ...
          <td>
            <div class="sup-actions">
              <button class="btn-icon sup-edit-btn" title="邱ｨ髮・>
                <span class="material-symbols-outlined">edit</span>
              </button>
              <button class="btn-icon sup-delete-btn" title="蜑企勁">
        ...
        <div class="mobile-card" data-id="${s.id}" style="cursor: default;">
          ...
          <div class="mobile-card-main">
            ...
            <div class="mobile-card-sub">
              ...
              ${s.contactPerson ? `<span>ﾂｷ 諡・ｽ・ ${s.contactPerson}</span>` : ''}
            </div>
            <div class="mobile-card-sub" style="margin-top: 4px;">
              ${s.phone ? `<span>到 ${s.phone}</span>` : ''}
              ${s.rating ? `<span style="margin-left: 8px;">${renderStars(s.rating)}</span>` : ''}
            </div>
            ${s.address ? `<div class="mobile-card-sub" style="font-size: 11px; margin-top: 2px;">桃 ${s.address}</div>` : ''}
          </div>
          <div class="mobile-card-end" style="flex-direction: row; align-items: center; gap: 8px;">
            ${s.website ? `
              <a href="${s.website}" target="_blank" rel="noopener" class="btn-icon" title="繧ｵ繧､繝医ｒ髢九￥" onclick="event.stopPropagation()" style="min-width: 40px; min-height: 40px; display: flex; align-items: center; justify-content: center;">
                <span class="material-symbols-outlined" style="color:var(--md-primary);">open_in_new</span>
              </a>
            ` : ''}
            <button class="btn-icon sup-edit-btn" title="邱ｨ髮・ style="min-width: 40px; min-height: 40px;">
              <span class="material-symbols-outlined">edit</span>
            </button>
            <button class="btn-icon sup-delete-btn" title="蜑企勁" style="min-width: 40px; min-height: 40px;">
  ...
  showAddModal(editItem = null) {
    ...
    const title = isEdit ? '莉募・繧悟・繧堤ｷｨ髮・ : '莉募・繧悟・繧定ｿｽ蜉';

    const bodyHtml = `
      <form id="supForm" class="sup-form">
        <div class="form-row">
          <div class="form-group">
            <label class="form-label required">莨夂､ｾ蜷・/label>
            <input type="text" class="form-input" name="name" required placeholder="萓・ 繝槭ャ繝励き繝｡繝ｩ">
          </div>
          <div class="form-group">
        ...
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">諡・ｽ楢・/label>
            <input type="text" class="form-input" name="contactPerson" placeholder="萓・ 螻ｱ逕ｰ螟ｪ驛・>
          </div>
          <div class="form-group">
            <label class="form-label">繝｡繝ｼ繝ｫ</label>
            <input type="email" class="form-input" name="email" placeholder="萓・ info@example.com">
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">髮ｻ隧ｱ逡ｪ蜿ｷ</label>
            <input type="text" class="form-input" name="phone" placeholder="萓・ 03-1234-5678">
          </div>
          <div class="form-group">
            <label class="form-label">Web繧ｵ繧､繝・/label>
            <input type="url" class="form-input" name="website" placeholder="萓・ https://example.com">
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">菴乗園</label>
          <input type="text" class="form-input" name="address" placeholder="萓・ 譚ｱ莠ｬ驛ｽ譁ｰ螳ｿ蛹ｺ...">
        </div>
        <div class="form-group">
          <label class="form-label">隧穂ｾ｡</label>
          <select class="form-select" name="rating">
            <option value="">譛ｪ隧穂ｾ｡</option>
            <option value="1">笘・・笘・・笘・(1)</option>
            <option value="2">笘・・笘・・笘・(2)</option>
            <option value="3">笘・・笘・・笘・(3)</option>
            <option value="4">笘・・笘・・笘・(4)</option>
            <option value="5">笘・・笘・・笘・(5)</option>
          </select>
        </div>
        <div class="form-group">
    ...
    const footerHtml = `
      <button class="btn btn-text" id="supFormCancel">繧ｭ繝｣繝ｳ繧ｻ繝ｫ</button>
      <button class="btn btn-filled" id="supFormSave">${isEdit ? '譖ｴ譁ｰ' : '霑ｽ蜉'}</button>
    `;
    ...
    setTimeout(() => {
      ...
      $('#supFormSave')?.addEventListener('click', () => {
        ...
        if (isEdit) {
          ...
          showSnackbar('莉募・繧悟・繧呈峩譁ｰ縺励∪縺励◆', 'success');
        } else {
          store.add('suppliers', data);
          showSnackbar('莉募・繧悟・繧定ｿｽ蜉縺励∪縺励◆', 'success');
        }
        hideModal();
        this.loadData();
      });
